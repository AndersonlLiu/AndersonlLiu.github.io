...
function Profile({ setRegister, currentUser }) {
    ...
    const handleChangePassword = async () => {
        try {
            ...
            const response = await fetch(`http://localhost:3031/posts/users/${currentUser}/password`, {
                method: 'PUT',
                headers: {
    ...
    const handleDeleteAccount = async () => {
        try {
            const response = await fetch(`http://localhost:3031/posts/users/${currentUser}/delete`, {
                method: 'DELETE'
            });
